...
impl MyApiForApplyDelete {
    ...
    pub async fn delete_from_yaml(&self, yaml: &str) -> Result<(), anyhow::Error> {
        let values = multidoc_deserialize(yaml)?;
        //从yaml中获取有用信息并调用删除
        for value in values {
            let kind = value.get("kind").expect("get yaml kind error");
            let ns = value
                .get("metadata")
                .expect("get metadata error")
                .get("namespace")
                .ok_or_else(|| "default");
            let name = value
                .get("metadata")
                .expect("get metadata error")
                .get("name")
                .expect("get metadata.name error");
            if let Ok(namespaces) = ns {
                //有命令空间，进行删除
                self.delete(
                    kind.as_str().unwrap(),
                    namespaces.as_str().unwrap(),
                    name.as_str().unwrap(),
                )
                .await
                .unwrap();
            ...
            if let Err(err) = ns {
                //无命名空间，默认空间进行删除
                self.delete(kind.as_str().unwrap(), err, name.as_str().unwrap())
                    .await
                    .unwrap();
            }
        }
...
spec:
  ...
  hostPath:
    ...
    for oc in svec {
        let p1 = oc.get("kisnd").ok_or_else(|| "default");
